#This version Impliments no scheduling or priority system in the program.
...
while (True):
    ...
    if(any(objects)):
        ...
        if(distance < 350000):
            ...
            print("Folders In System: %d" %NumOfFaces)
            captureNewFace()
            faceRecognized = False
    ...
    # else: #if no faces or rectangles are detected turn light white
    if(foundStuff == 2):
        foundStuff = 0
        green_led.on()
        red_led.on()
